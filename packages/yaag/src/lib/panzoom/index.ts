...
import {animate} from '../amator/index';
...
export function createPanZoom(domElement, options = {}) {
  ...
  function showRectangle({right, left, bottom, top}) {
    ...
    transform.x = -(left + rectWidth / 2) * scale + size.x / 2;
    transform.y = -(top + rectHeight / 2) * scale + size.y / 2;
  ...
  function handleSingleFingerTouch({touches}) {
  ...
  function handleTouchEnd({touches}) {
    if (touches.length > 0) {
      var offset = getOffsetXY(touches[0]);
      const point = transformToScreen(offset.x, offset.y);
      mouseX = point.x;
      mouseY = point.y;
    } else {
  ...
  function getOffsetXY({clientX, clientY}) {
    let offsetX;
    let offsetY;
    // I tried using e.offsetX, but that gives wrong results for svg, when user clicks on a path.
    const ownerRect = owner.getBoundingClientRect();
    offsetX = clientX - ownerRect.left;
    offsetY = clientY - ownerRect.top;
...
function autoRun() {
  ...
  function collectOptions({attributes}) {
